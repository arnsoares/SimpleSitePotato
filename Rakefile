...
def get_compile_line(input, output, exclude)
    ...
    line = "mxmlc #{escape(File.join(current_path, input))} -o=#{escape(File.join(current_path, output))} -debug=#{config['default'][0]['debug']} #{libs_path} #{sources_path} #{config['default'][0]['extras']}"
